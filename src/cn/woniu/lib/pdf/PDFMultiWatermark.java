...
/** 
 * @ClassName: PDFMultiWatermark <br/> 
 * @Description: TODO  <br/> 
 * 
 * @author woniu1983 
 * @date: 2018年10月17日 下午5:30:43 <br/>
 * @version  
 * @since JDK 1.6 
 */
public class PDFMultiWatermark extends PDFWriter {
	 ...
	 *    多个水印图片及其配置
	 */
	private ArrayList<WMarkImage> wmarks = new ArrayList<WMarkImage>();
	
	public PDFMultiWatermark(final PDFReader reader, final File saveFile, WMarkImage... markimgs) throws IOException {
		super(saveFile);
		this.reader = reader;
		this.prevxref = this.reader.getLastXref();
		this.body = new PDFBody(this);
		if (markimgs == null || markimgs.length <= 0) {
			throw new IOException("Missing WMarkImage instances.");
		}
		
		wmarks.clear();
		for (WMarkImage wm : markimgs) {
			if (wm.image != null) {
				wmarks.add(wm);
			}			
		}
	...
	private void writeImages() throws IOException {
		...
		int n = 0;
		for (WMarkImage wm : wmarks) {
			n++;
			Logger.Debug("#####PDF---writeImages--Add Mark[" + n + "]");
			if (wm.pageMode == PageMode.FIRST) {
				begin = 1;
				last = 1;
			} else if (wm.pageMode == PageMode.LAST) {
				begin = pages;
				last = pages;
			} else {
				//TODO
			...
			//调整WM位置， 根据第一页的长和宽
			adaptWMPosition(begin, wm);

			for(int i = begin; i <= last; i++) {
				PageStamp stamp = getPageStamp(i);

				float matrix[] = wm.image.matrix();
				matrix[PDFImage.CX] = wm.image.getAbsoluteX() - matrix[PDFImage.CX];
				matrix[PDFImage.CY] = wm.image.getAbsoluteY() - matrix[PDFImage.CY];
				addImage(wm.image, stamp, matrix[0], matrix[1], matrix[2], matrix[3], matrix[4], matrix[5]);
			}
		}

	}

	private void adaptWMPosition(int speciedPageNum, final WMarkImage wm) {
		// 度数--对原始图像，设定一次即可, 多次设定可能导致度数叠加
		wm.image.setRotationDegrees(wm.rotateDegree);

		float imgW = wm.image.getScaledWidth();
		float imgH = wm.image.getScaledHeight();
		...
		if (imgW >= pageW || imgH >= pageH) {
			wm.image.setAbsolutePosition(0f, 0f);
			return;
		}
		...
		wm.image.setAbsolutePosition(posW, posH);
	}
